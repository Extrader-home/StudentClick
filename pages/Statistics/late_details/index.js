...
Page({
  ...
  onLoad: function (){
    var that = this;
    var d_w_m = wx.getStorageSync('d_w_m') || 'day';
    ...
    return new Promise(function (resolve, reject){
      wx.request({
        url: list[0].api_url+'late/late_details.php',
        data: {
          d_w_m: d_w_m,
        ...
        success: function(res){  
          that.data.late_detail_list = res.data;
          that.setData({
            late_detail_list: that.data.late_detail_list,
            late_detail_list_length: that.data.late_detail_list.length
          });
        },
        fail: function(fail) {  
          Dialog.confirm({
  ...
  send_delete(student_id, time){
    return new Promise(function (resolve, reject){
      wx.request({
        url: list[0].api_url+'delete/delete_late.php',
        data: {
